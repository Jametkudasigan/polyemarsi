"""Polymarket API Client - CLOB V2 Support"""
import requests
import json
import time
...
class PolymarketClient:
    ...
    def init_clob(self):
        """Inisialisasi CLOB V2 client dan auto-generate API credentials"""
        ...
        try:
            ...
            print(f"[Polymarket] CLOB init error: {e}")
            raise

    def get_balance(self) -> float:
        """Get PolyUSD balance di Polymarket V2"""
        ...
        try:
            ...
            # V2 pakai PolyUSD, return dalam format decimal
    ...
    def check_allowance(self) -> bool:
        """Cek apakah allowance sudah cukup untuk trading"""
        if self._clob is None:
            self.init_clob()
        try:
            bal = self._clob.get_balance_allowance(
